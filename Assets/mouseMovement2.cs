...
public class mouseMovement2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        collectibles = GameObject.FindGameObjectsWithTag("cookieplate").Select(obj => obj.transform).ToArray();
    }
    ...
    void Move()
    {
        ...
        Debug.Log("Move Input: " + moveInput); // Debug log for move input
        Vector2 targetVelocity = new Vector2(moveInput * moveSpeed, rb.velocity.y);
        Debug.Log("Target Velocity: " + targetVelocity); // Debug log for target velocity
    ...
    {
        if (other.CompareTag("cookieplate"))
        {
